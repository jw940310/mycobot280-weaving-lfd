# DMP 연동 및 스킬 학습 (W4 07.22~07.23)

합성 위빙 궤적(W3)을 DMP로 학습·재현하기 위한 `movement_primitives` 연동
(07.22)과 shape별 스킬 모델 학습(07.23) 기록. 파라미터 추출(07.24)의 출발점.
...
## 4. DMP 스킬 모델 학습 (07.23)

shape당 프로토타입 DMP 스킬 하나를 학습해 `.npz`로 저장한다.

```bash
ros2 run mycobot_280_lfd_dmp train_dmp --shape all   # → data/models/{shape}.npz
```

### medoid를 쓰고 가중치 평균을 안 쓰는 이유

위빙은 진동 운동이고 시연마다 주파수가 지터로 다르다(이 데이터셋은 약 15%).
위상 공간에서 여러 시연의 forcing weights를 **평균하면 어긋난 진동이 겹쳐
진폭이 뭉개진다**(측정상 약 9% 감쇠). 그래서 평균 대신 **medoid** — 위빙
파라미터(진폭·주파수·속도)가 데이터셋 평균에 가장 가까운 대표 시연 — 를 골라
그 하나로 학습한다. 프로토타입은 '깨끗한 정본 위빙'이고, 진폭·주파수 같은
변이는 07.24에서 파라미터로 뽑아 변조한다.

### 학습 결과 (shape당 시연 10건, `n_weights_per_dim=30`)

| shape | medoid | 자기적합 RMSE | 진폭 충실도 |
|---|---|---|---|
| line | line_0010 | 0.24 mm | 99% |
| zigzag | zigzag_0003 | 0.25 mm | 91% |
| crescent | crescent_0006 | 0.30 mm | 104% |

(zigzag 91%는 삼각파의 날카로운 꼭짓점을 30가중치 DMP가 약간 둥글리기 때문.)

### 품질 지표 주의: 재타겟 pose RMSE

프로토타입을 각 시연의 시작/끝점·시간으로 재타겟해 pose 점별 RMSE를 재면
2.5~4.5 mm로 크게 나온다. 이는 **모델 결함이 아니라** 시연 간 위빙 주파수
차이로 생기는 위상 드리프트다 — 점별 RMSE는 주파수가 다른 진동을 비교하는 데
부적합하다(단일 medoid를 재타겟해도 같은 값이 나온다). 모델 충실도는 medoid
자기적합 RMSE와 진폭 보존으로 판단하고, 주파수까지 정렬한 비교는 07.24
파라미터 추출로 넘긴다.

### 모델 파일 (`model.py`)

`movement_primitives` 객체를 pickle하지 않고 순수 배열(.npz) + JSON 메타로
저장한다 — 라이브러리 버전에 안 묶이고 사람이 열어볼 수 있게. `DMPSkill.load`
→ `to_dmp()`로 되살리며, 가중치 주입 재생이 원본 학습 DMP와 비트 단위로
일치함을 확인했다. 저장 포맷은 `data/`(gitignore)에 두고 코드+seed로 재생성
한다.

RViz 확인: `dmp_compare.launch.py`에 데모=medoid, 재현=프로토타입 CSV를 물리면
전 구간 포개진다(정본 위빙 재현).
